...
- ESTRUCTURA POR FAMILIA: la imprenta tiene plantillas de ítems típicos por familia de producto. Catálogo: ${JSON.stringify(ctx.estructuras || [])}. Si el pedido encaja con una familia, poné su nombre en "familia" y devolvé UN item por cada pieza/insumo/proceso de esa estructura (respetá los fijos; sumá los opcionales sólo si el pedido los menciona). En cada item poné "rol" = pieza | insumo | proceso | manoobra según corresponda. Ejemplos del rubro: una BOLSA se compone de cuerpo + fondo + (refuerzo opcional) + manija (cordón/cinta) + armado; un ESTUCHE = cartulina impresa + troquel + troquelado + pegado (+ laminado/hot stamping/laca si se piden); una REVISTA = tapa + interiores (múltiplo de 4 páginas) + abrochado (+ laminado de tapa). Si el pedido es un solo producto simple, devolvé un único item sin "familia".
